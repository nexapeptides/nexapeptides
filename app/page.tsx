...
export default function NexaPeptidesPage() {
  ...
  // This will be filled by the shopper
  const [discountCode, setDiscountCode] = useState("");

  // Codes loaded from localStorage (set via admin dashboard)
  const [approvedCodes, setApprovedCodes] = useState<string[]>([
    // fallback defaults if nothing in localStorage yet:
    "KENNY10",
    "JAY10",
    "SARAH10",
    "COACHK",
  ...
  // Load approved codes from localStorage on first render
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed)) {
          setApprovedCodes(parsed.map((c: string) => c.toUpperCase()));
        }
      }
    } catch (err) {
  ...
  const isCodeValid = useMemo(() => {
    ...
    return approvedCodes.includes(normalized);
  }, [discountCode, approvedCodes]);

  const discountAmount = useMemo(() => {
    return isCodeValid ? subtotal * 0.1 : 0;
  }, [subtotal, isCodeValid]);
...
Discount Valid: ${isCodeValid ? "YES" : "NO"}
Discount Amount: $${discountAmount.toFixed(2)}
Total After Discount: $${finalTotal.toFixed(2)}

NOTE TO OWNER:
If Discount Valid == YES, pay 10% commission to the owner of that code.
      ...
      <header className="border-b border-neutral-200 sticky top-0 bg-white z-40">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* (hidden) admin link - you can remove this if you don't
                want ANYONE guessing the URL, but it's convenient for you */}
            <a
              href="/admin-codes"
              className="text-[10px] text-neutral-400 underline hover:text-neutral-800"
            >
              Admin
            </a>
                ...
                <>
                  ...
                  <div className="mb-3">
                    <input
                      ...
                      className="w-full border border-neutral-300 rounded-xl px-3 py-2 text-sm uppercase"
                    />
                    {discountCode && (
                      <p
                        className={`text-xs mt-1 ${
                          isCodeValid ? "text-green-600" : "text-red-500"
                        }`}
                      >
                        {isCodeValid
                  ...
                  <div className="text-right text-sm mb-4 space-y-1">
                    ...
                    >
                      <span>
                        Discount {discountCode ? `(${discountCode.toUpperCase()})` : "(none)"}
                      </span>
